...
def scan():
    # Recebe as instruções do seu site original
    payload = request.get_json(force=True, silent=True) or {}
    api_key = ENV_API_KEY or (payload.get("apiKey") or "").strip()
    
    # Chama o seu scanner original (scanner.py)
    result = run_scan(
        api_key=api_key,
    ...
    # RETORNA DADOS (JSON): Isso faz o seu site preencher as tabelas e não mostrar texto
